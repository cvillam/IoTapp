...
{
    public partial class Conocimiento13 : PhoneApplicationPage
    {
        ...
        public Conocimiento13()
        {
            ...
            if (x == 0)
            {
                Question.Text = "Se desea iniciar una conexión para enviar datos mediante UDP. ¿Qué función  haría esta acción?";
                RadioA.Content = "UDP.sendPacket()";
                RadioB.Content = "UDP.beginPacket()";
                RadioC.Content = "UDP.initCon()";
                RadioD.Content = "UDP.connect()";
                rcorrecta = "B";

            }
            else if (x == 1)
            {
                Question.Text = "Para desconectar o finalizar la conexión con  un servidor se usa la función: ";
                RadioA.Content = "Close();";
                RadioB.Content = "Exit();";
                RadioC.Content = "ExitConnection();";
                RadioD.Content = "Stop();";
                rcorrecta = "D";
            }
                         ...
                         if (IsolatedStorageSettings.ApplicationSettings.Contains(FILE_NAME))
                            {
                                ...
                                IsolatedStorageSettings.ApplicationSettings[FILE_NAME] = "14";
                             }
                             else {
                             IsolatedStorageSettings.ApplicationSettings.Add(FILE_NAME, "14");

                                }
                    MessageBox.Show("Correcto!, Has avanzado al nivel 14 de 20");
                    NavigationService.Navigate(new Uri("/PreguntasConocimiento/Conocimiento14.xaml", UriKind.Relative));
                }
                else {
                    int intento;
